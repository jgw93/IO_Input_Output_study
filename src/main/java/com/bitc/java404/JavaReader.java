...
public class JavaReader {
    ...
    public void read1() throws Exception {
        ...
        // Reader 클래스는 추상 클래스이므로 Reader 클래스를 상속받은 FileReader 클래스 타입의 객체를
        // Reader 타입의 변수에 저장하여 사용함
        Reader reader = new FileReader("C:\\java404\\test2.txt");
        int readDate; // 읽어온 문자를 저장할 변수
        ...
        reader.close(); // Reader 도 외부 리소스이기 때문에 사용 후 반드시 닫아줘야 한다.
    }
    ...
    public void read3(String filename) throws Exception {
        ...
        char[] cbuff = new char[4]; // 파일에서 읽어온 데이터를 저장할 charp[] 배열
        // read(char[] cBuff, int off, int len) 을 사용하여 파일에서 데이터를 읽어와서
        // 지정한 배열에 저장 시 offset 으로 지정한 위치에 length 크기만큼 저장함
